...
MODULE_PARM_DESC(alloc_size, "Size of allocated memory (Default=64)");
...
__init int init_module(void)
{
  ...
  if (ptr != NULL)
  {
    ...
    {
      ptr[i] = kmalloc(sizeof(char *), GFP_KERNEL);
    }
    uint64_t stop = setRDTSC();

    uint64_t avgTime = (stop - start) / loop_cnt;
    printk(KERN_INFO "Average Malloc-Time for %d iterations with the size of %d bytes: %d\n", loop_cnt, alloc_size, avgTime);
  }
  else
  {
    printk(KERN_INFO "Memory allocation not successful!\n");
  }
  return 0;
}
